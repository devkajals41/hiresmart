					...
					>
						<div>
							...
							<p className="text-[13.5px] text-slate-500 font-medium mt-1">
								Review your core parameters, checklist milestones, and
					...
					>
						<div className="grid grid-cols-1 md:grid-cols-12 gap-6.5 items-center">
							...
							<div className="md:col-span-5 text-center md:text-left space-y-3.5">
								<div className="space-y-1">
									...
									<p className="text-[13.5px] text-slate-500 font-medium leading-relaxed">
										You demonstrated solid domain familiarity. Review your
										performance markers below to enhance your candidate rating
										index.
									</p>
								</div>
							...
							<div className="md:col-span-3 flex justify-center md:justify-end pr-1 h-36">
								<svg
									className="w-full max-w-[200px] h-full overflow-visible"
									viewBox="0 0 200 140"
								>
									<defs>
						...
						>
							...
							<div className="mt-5 pt-3.5 border-t border-slate-100 bg-slate-50/50 -mx-6.5 -mb-6.5 p-4 rounded-b-3xl flex items-start gap-2.5">
								...
								<p className="text-[12px] text-slate-500 leading-normal">
									Values represent weighted semantic ratings calculated from
									your verbal clarity and tech depth benchmarks.
								</p>
							</div>
						</motion.div>
									...
									<div className="sm:col-span-8 space-y-3">
										{strengths.length > 0 ? (
											strengths.map((s, i) => (
												<div
													key={i}
													className="flex items-start gap-2.5 text-[13px] font-semibold text-slate-650"
												>
													<Check
														size={14}
														className="text-emerald-500 stroke-[3.5] shrink-0 mt-1"
													/>
													<span>{s}</span>
												</div>
											))
							...
							{weaknesses.length > 0 ? (
								weaknesses.slice(0, 2).map((w, i) => {
									...
									return (
										...
										>
											<div
												className={`p-2.5 ${c.bg} ${c.text} border ${c.border} rounded-xl shrink-0`}
											>
												<Icon size={16} />
											</div>
											<div>
								...
								<div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/50 flex gap-3.5 items-start lg:col-span-2">
									...
									<div>
										...
										<p className="text-[12px] text-slate-500 mt-1 leading-normal font-semibold">
											Continue practising to sharpen your interview skills
											further.
										</p>
									</div>
								</div>
							...
							<div className="space-y-4">
								{breakdown.length > 0 ? (
									breakdown.map((item, idx) => (
										<div
											key={idx}
											className="rounded-2xl border border-slate-200 bg-white p-4.5 space-y-3.5"
										>
