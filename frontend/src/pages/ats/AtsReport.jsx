...
function ScoreBar({ label, score, icon: Icon }) {
	...
	return (
		<div className="flex items-center gap-4 py-2.5 px-4 rounded-2xl hover:bg-slate-50/70 hover:shadow-[0_4px_12px_rgba(0,0,0,0.015)] transition-all duration-300">
			...
			<span
				className={`text-[14px] font-black w-8 text-right font-sans ${numColor}`}
			>
				{score}
			</span>
		</div>
		...
		>
			<div className="flex items-center justify-between gap-2.5 mb-5 pb-3.5 border-b border-slate-150/60">
				<div className="flex items-center gap-2.5">
					...
					<h3 className="text-[15.5px] font-black text-slate-800 tracking-tight">
						{title}
					</h3>
				</div>
				{extraHeader}
...
export default function AtsReport() {
	...
	const getDynamicGrade = (score) => {
		if (score >= 90)
			return {
				label: "Excellent Portfolio",
				style: "text-emerald-800 bg-emerald-50 border-emerald-200",
			};
		if (score >= 80)
			return {
				label: "Highly Competitive",
				style: "text-teal-800 bg-teal-50 border-teal-200",
			};
		if (score >= 65)
			return {
				label: "Strong Match",
				...
				>
					...
					>
						<div>
							...
							<p className="text-[13.5px] text-slate-500 font-medium mt-1">
								Refine, add keywords, and evaluate layout compliance in
								real-time.
							</p>
						</div>
					...
					>
						<div
							...
							style={{
								backgroundImage:
									"radial-gradient(circle, #0f766e 1.2px, transparent 1.2px)",
									...
									<p className="text-[13.5px] text-slate-500 font-medium leading-relaxed max-w-sm">
										{customScore > report.overall_score ? (
											<span className="text-emerald-700 font-bold">
												Great job! You added relevant keywords boosting your
												match index by{" "}
												{(customScore - report.overall_score).toFixed(0)}%.
												Update your source resume with these items.
											</span>
										) : (
							...
							>
								...
								>
									<div className="space-y-1.5 py-2">
										{Object.entries(report.section_scores).map(
											([name, score]) => (
												<ScoreBar
													key={name}
													label={name}
									...
									<div className="flex flex-col justify-between h-full gap-5">
										...
										<div className="bg-emerald-50/40 border border-emerald-100/50 rounded-xl p-3.5 flex items-start gap-2.5">
											...
											<p className="text-[12.5px] text-slate-600 leading-normal">
												These are keywords successfully parsed from your PDF.
												Use the{" "}
												<strong className="text-slate-800">
													Keyword Optimizer
												</strong>{" "}
												tab to insert missing tools.
											</p>
										</div>
									</div>
								</PremiumCard>
								...
								<div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
									...
									<PremiumCard
										...
										iconBg="bg-red-50 border border-red-150 text-red-500"
									>
								...
								<div className="md:col-span-3 rounded-2xl border border-white/60 bg-white/70 backdrop-blur-xl p-5 shadow-lg flex flex-col gap-4">
									<div className="flex items-center justify-between pb-3 border-b border-slate-100 gap-4">
										<h3 className="text-[15px] font-black text-slate-800">
											High-Impact Suggested Keywords
										</h3>
								...
								<div className="md:col-span-2 rounded-2xl border border-white/60 bg-white/70 backdrop-blur-xl p-5 shadow-lg flex flex-col justify-between gap-5 h-full">
									<div>
										...
										<p className="text-[12.5px] text-slate-500 mt-3 leading-relaxed">
											By adding keywords, you simulate tailoring your resume for
											target positions. These items must be logically added into
											your actual projects or descriptions.
										</p>
														...
														>
															...
															<button
																...
																onClick={(e) => {
																	e.stopPropagation();
																	setAddedKeywords(
																		addedKeywords.filter((k) => k !== kw),
																	);
																	const refBonus =
																		RELEVANT_KEYWORDS.find((k) => k.name === kw)
																			?.scoreBonus || 2;
																	setCustomScore((prev) =>
																		prev - refBonus < 76 ? 76 : prev - refBonus,
																	);
																}}
															>
																&times;
									...
									<div className="pt-3 border-t border-slate-100 flex items-center justify-between">
										...
										<span className="text-[18px] font-black text-emerald-850">
											{customScore} / 100
										</span>
									</div>
								</div>
													...
													},
													{
														...
														status:
															...
															report.weaknesses.some(
																(w) =>
																	w.toLowerCase().includes("quantif") ||
																	w.toLowerCase().includes("number") ||
																	w.toLowerCase().includes("metric"),
															)
																? "warning"
																: "pass",
														detail:
