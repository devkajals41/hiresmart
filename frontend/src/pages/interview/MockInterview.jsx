...
const TECH_TOPICS = [
	{
		...
		icon: Code2,
		color: "hover:border-emerald-500 hover:shadow-glow-emerald",
	...
	},
	{
		...
		name: "System Design",
		icon: Layers,
		color: "hover:border-blue-500 hover:shadow-glow-blue",
	...
	},
	{
		...
		icon: Cpu,
		color: "hover:border-amber-500 hover:shadow-glow-amber",
	...
	},
	{
		...
		icon: Globe,
		color: "hover:border-purple-500 hover:shadow-glow-purple",
	...
	},
	{
		...
		icon: Database,
		color: "hover:border-cyan-500 hover:shadow-glow-teal",
		iconColor: "text-cyan-700 bg-cyan-50 border border-cyan-100",
	},
	{
		id: "ml-ai",
		name: "Machine Learning & AI",
		icon: Brain,
		color: "hover:border-pink-500 hover:shadow-glow-rose",
		iconColor: "text-pink-700 bg-pink-50 border border-pink-100",
	},
	{
		id: "web-dev",
		name: "Web Development",
		icon: Laptop,
		color: "hover:border-rose-500 hover:shadow-glow-rose",
	...
	},
	{
		...
		name: "DevOps & Cloud",
		icon: Cloud,
		color: "hover:border-indigo-500 hover:shadow-glow-blue",
...
const DICTATED_ANSWERS = {
	"Data Structures & Algorithms":
		"To optimize runtime search, we implement a balanced binary search tree or hash table. Under hash maps, key collision resolutions can be handled by chaining using linked lists. This achieves an average O(1) complexity for write and lookup methods, significantly beating nested loops which scale quadratic at O(N^2) in execution cost.",
	"System Design":
		"For high-traffic distributed services, we enforce stateless gateway layouts. Session caches are distributed across Redis pools, and active APIs are throttled using a token bucket rate limiter. Writes are routed to a master transactional database, while read query weights are balanced dynamically across multiple read replicas.",
	"Object-Oriented Programming":
		"OOP architectures rely on dependency inversion principles, allowing high-level modules to interact via abstractions rather than concrete structures. By satisfying the single responsibility guideline, we separate logical domains, using object composition instead of fragile base class extensions to maintain decoupling.",
	"Computer Networks":
		"TCP connections establish reliable streams by implementing a three-way state handshake: sending a SYN flag, receiving a SYN-ACK validation, and responding with an ACK token. This state handshake tracks sequence numbers and handles packet retransmission, preventing network package dropouts unlike connectionless UDP streaming.",
	"Database Management Systems":
		"Database engines satisfy transactional ACID consistency by implementing write-ahead logs and isolation levels. To optimize indexing costs, we create indexes on query foreign filters. We partition heavy data sets using horizontal range sharding, distributing indexing sizes and search times.",
	"Machine Learning & AI":
		"Model parameters adjust their weights using backward propagation, computing the derivative loss gradient. Under text sequences, attention values weigh relationships in parallel across tokens, allowing modern transformers to parse contextual variables far better than older sequential models.",
	"Web Development":
		"Web rendering engines calculate visual tree elements sequentially. React bypasses redundant recalculation costs by computing tree variations in a Virtual memory DOM. It compares the changed node states against previous models and batches minimum repaint changes, keeping browser thread layouts fast.",
	"DevOps & Cloud":
		"DevOps pipelines treat environment servers as immutable infrastructure templates. We deploy containers using Docker to establish identical execution paths. Staging images are compiled automatically via CI configuration runners and deployed to Amazon ECS instances once unit tests complete.",
};
...
export default function MockInterview() {
	...
	const activeTopicObj =
		TECH_TOPICS.find((t) => t.name === selectedTopic) || TECH_TOPICS[0];
	...
	const handleStartInterview = async () => {
		...
		const pool =
			TOPIC_QUESTIONS[selectedTopic] ||
			TOPIC_QUESTIONS["Data Structures & Algorithms"];
	...
	const handleToggleVoice = () => {
		if (isRecording) {
			...
			const simulatedText =
				DICTATED_ANSWERS[selectedTopic] || "Sample dynamic answer recorded.";
			const newAnswers = [...answers];
			newAnswers[currentIdx] = simulatedText;
			setAnswers(newAnswers);

			toast.success("Voice transcript synthesized successfully!", {
				icon: "🎤",
			});
		} else {
			setIsRecording(true);
			setRecordingSeconds(0);
			toast.success("Recording answer... Speak clearly into microphone.", {
				icon: "🎙️",
			});
	...
	const handleEndInterview = () => {
		if (
			window.confirm(
				"Are you sure you want to end the session? You will be evaluated on the answers provided so far.",
			)
		) {
			handleSubmitInterview();
		}
		...
		try {
			...
			const demoFeedback = {
				...
				breakdown: questions.map((q, idx) => ({
					...
					comments:
						"Accurate definition. Try adding specific real-world implementation examples for maximum scoring.",
					model_answer:
						DICTATED_ANSWERS[selectedTopic] || "Sample model answer.",
				})),
			};
						...
						>
							<div>
								<div className="flex items-center gap-2">
									...
									<span className="text-[12px] font-black uppercase text-emerald-800 tracking-wider">
										AI Coach
									</span>
								</div>
								...
								<p className="text-[13.5px] text-slate-550 font-medium mt-1">
									Establish parameters, choose a focus topic, and analyze
							...
							<div className="grid grid-cols-2 md:grid-cols-5 gap-4 p-4.5 rounded-2xl bg-white border border-slate-200/70 shadow-sm relative overflow-hidden">
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<span className="text-[12.5px] font-black text-slate-700 block truncate max-w-[120px]">
											{role}
										</span>
									</div>
								</div>
								...
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<span className="text-[12.5px] font-black text-slate-700 block font-mono">
									...
									<div className="rounded-3xl border border-white/60 bg-white/85 backdrop-blur-xl p-6 shadow-xl shadow-slate-100/50 space-y-5">
										...
										<div className="space-y-2 pt-1 pb-3">
											...
											{getHintsForQuestion(
												...
												<div
													key={idx}
													className="flex items-center gap-2.5 text-[13px] font-bold text-slate-600"
												>
													<CheckCircle
														size={14}
												...
												{isRecording && (
													<div className="absolute inset-0 bg-slate-900/95 rounded-2xl flex flex-col items-center justify-center text-white p-5 backdrop-blur-sm">
														...
														<p className="text-[13px] font-bold text-slate-100">
															Microphone Active — Dictating Response
														</p>
										...
										<div className="flex justify-between items-center text-[12.5px] text-slate-400 font-bold border-t border-slate-100 pt-3">
											...
											<span>
												...
												{answers[currentIdx]?.trim()
													? answers[currentIdx].trim().split(/\s+/).length
													: 0}
											</span>
										</div>
									...
									<div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm space-y-4">
										...
										<div className="space-y-3">
											...
											<div className="flex items-start gap-2.5 text-[12.5px] font-semibold text-slate-600 leading-normal">
												...
												/>
												<span>
													Structure your reply logically (Intro, Body,
													Conclusion).
												</span>
											</div>
											<div className="flex items-start gap-2.5 text-[12.5px] font-semibold text-slate-600 leading-normal">
												...
												/>
												<span>
													Be concise and target technical specifications.
												</span>
											</div>
											<div className="flex items-start gap-2.5 text-[12.5px] font-semibold text-slate-600 leading-normal">
												<Check
												...
												/>
												<span>
													Illustrate designs with realistic code samples.
												</span>
											</div>
										</div>
									</div>
										...
										<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
											<div className="relative">
												...
												>
													...
													<ChevronDown
														size={15}
														className={`text-slate-455 transition-transform ${showRoleDropdown ? "rotate-180" : ""}`}
													/>
												</button>
												<AnimatePresence>
													{showRoleDropdown && (
											...
											<div className="relative">
												<label className="text-[12px] font-extrabold text-slate-455 uppercase tracking-widest block mb-2">
													Question Count
												</label>
												<button
													onClick={() =>
			...
			loadingInterview
				...
				: "bg-gradient-to-r from-emerald-650 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-lg shadow-emerald-700/10"
								...
								>
									...
									<div>
										...
										<div className="flex flex-col items-center justify-center text-center p-4 bg-slate-50/60 rounded-2xl border border-slate-200/50">
											<div
												className={`p-3 rounded-2xl ${activeTopicObj.iconColor} shadow-sm shrink-0 mb-3`}
											>
												<TopicIcon size={28} />
											</div>
											...
											<h4 className="text-[16px] font-black text-slate-805 leading-tight mt-1.5">
												{selectedTopic}
											</h4>
										</div>
									</div>
									...
									<div className="flex items-center justify-between pb-4.5 border-b border-slate-155 mb-5 relative">
										<div>
											...
											<p className="text-[12px] text-slate-500 font-medium mt-0.5">
												Toggle tech-related categories
											</p>
										</div>
										<button
											onClick={() => setIsTopicModalOpen(false)}
											...
											return (
												<div
													...
													onClick={() => {
														...
														toast.success(`Domain focus set: "${top.name}"`, {
															icon: "🎯",
														});
													}}
									...
									>
										<div className="bg-slate-50/50 border-b border-slate-100 p-4.5 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
											<div className="flex items-start gap-3">
												...
												<p className="text-[13.5px] font-black text-slate-850 leading-normal">
													{item.question}
												</p>
											</div>
					...
					{!isInterviewing && !isEvaluating && !feedback && (
						<motion.div
							variants={itemVariants}
							className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-5 border-t border-slate-200/50"
						>
							<div className="flex items-center gap-3.5 p-3 rounded-2xl bg-white/50 border border-white/60 hover:bg-white hover:shadow transition-all duration-300">
								...
								<div className="leading-tight">
									<h6 className="text-[12.5px] font-black text-slate-750">
										Speech Synthesis
									</h6>
									<p className="text-[11px] text-slate-400 mt-1">
										Translate audio notes
									</p>
								</div>
							</div>
							<div className="flex items-center gap-3.5 p-3 rounded-2xl bg-white/50 border border-white/60 hover:bg-white hover:shadow transition-all duration-300">
